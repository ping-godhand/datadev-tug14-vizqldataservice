...
SYSTEM_PROMPT = """
You are a field specification generator for data visualizations.
...
YOUR TASK:
Convert natural language queries into field specifications.

FORMAT RULES:
- For Measures: {{"fieldCaption": "COLUMN_NAME", "function": "SUM", "maxDecimalPlaces": 2}}
- For Dimensions/Calculations: {{"fieldCaption": "COLUMN_NAME"}}

YOUR RESPONSE MUST BE A JSON ARRAY.

Example: For "Sales by Region" → 
[
...
formatted_system_prompt = SYSTEM_PROMPT.format(
    measures=", ".join(list_of_measures),
    dimensions=", ".join(list_of_dimensions),
    calculations=", ".join(list_of_calcs)
)
